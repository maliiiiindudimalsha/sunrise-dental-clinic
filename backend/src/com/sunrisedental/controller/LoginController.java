    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            if (username == null || username.trim().isEmpty()
                ...
                sendResponse(exchange, 400, "{\"status\":\"error\",\"message\":\"Username and password are required\"}");
                return;
            }

            User user;
            try {
                user = userService.login(username.trim(), password);
            } catch (IllegalArgumentException deactivated) {
                // Account exists and password is correct, but it's deactivated
                sendResponse(exchange, 403, "{\"status\":\"error\",\"message\":\""
                        + escapeJson(deactivated.getMessage()) + "\"}");
                return;
            }
